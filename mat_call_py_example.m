% This script contains examples of how to call Python functions from Matlab.
% Here, the Python functions are stored in TP_TN_OSC_funcs.py.
%
%
%
...
ret = py.TP_TN_OSC_funcs.compute_MC_matrix( py.numpy.array(A), int8(p) );
...
i = 3;
q = 5;
Lmat = py.TP_TN_OSC_funcs.L(int8(n), int8(i), int8(q));
reta = py.TP_TN_OSC_funcs.compute_MC_matrix( Lmat, int8(p) );
...
ret1 = py.TP_TN_OSC_funcs.compute_matrix_from_EB_factorization( valsL, valsD, valsU );
...
ret2 = py.TP_TN_OSC_funcs.EB_factorization_ITN( py.numpy.array( B ) );
...
isOSC = py.TP_TN_OSC_funcs.is_OSC( py.numpy.array(A), tol );
isTP = py.TP_TN_OSC_funcs.is_TP( py.numpy.array(A), tol );
isTN = py.TP_TN_OSC_funcs.is_TN( py.numpy.array(A) );
isITN = py.TP_TN_OSC_funcs.is_ITN( py.numpy.array(A), tol );
if( isTP )
  fprintf(1, 'A is TP.\n' );
elseif( isOSC )
  fprintf(1, 'A is osc.\n' );
...
B = py.TP_TN_OSC_funcs.compute_matrix_from_EB_factorization( valsL, valsD, valsU );
fprintf(1, 'B = \n' ); double(B)
isOSC = py.TP_TN_OSC_funcs.is_OSC( B, tol );
if( isOSC )
...
% ================================================
if( isOSC )
  r = py.TP_TN_OSC_funcs.osc_exp(B, tol);
  fprintf( 1, 'r(B) = %d.\n', r );
  %B_mat = double(B);
  %BB_mat = py.TP_TN_OSC_funcs.compute_MC_matrix( py.numpy.array(B_mat*B_mat), int8(3) );
  %fprintf( 1, '(B^2)^(3)=\n');
  %double(BB_mat{1})
end;
